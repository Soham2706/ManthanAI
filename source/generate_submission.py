...
# =====================================================
# SAVE AS EXCEL (<= 5 MB OPTIMIZED)
# =====================================================

import pandas as pd

output_path = "../outputs/generate_submission.xlsx"

# Convert to DataFrame (already created earlier as submission_df)
df = submission_df

# Reduce size (important for 5MB limit)
df["reasoning"] = df["reasoning"].astype(str).str[:250]  # trim long text

# Write Excel with compression-friendly settings
with pd.ExcelWriter(output_path, engine="xlsxwriter") as writer:
    df.to_excel(writer, index=False, sheet_name="ranking")

    # Optional: adjust column widths (keeps file clean, not heavier)
    workbook = writer.book
    worksheet = writer.sheets["ranking"]

    worksheet.set_column("A:A", 8)   # rank
    worksheet.set_column("B:B", 20)  # candidate_id
    worksheet.set_column("C:C", 10)  # score
    worksheet.set_column("D:D", 80)  # reasoning (trimmed anyway)

print("\n✅ Excel submission created!")
print("📁 Saved at:", output_path)
print("📦 Size should stay under 5MB (reasoning trimmed)")
